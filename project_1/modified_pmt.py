...
def fetch_stock_data(start, end, stock_list):
    df = fdr.DataReader(stock_list[0], start = start, end = end)[['Close']]
    df.columns = [stock_list[0]]
    for i in stock_list[1:]:
        temp_df = fdr.DataReader(i, start = start, end = end)[['Close']]
        temp_df.columns = [i]
...
def actual_return_plot(df, weight_list, timeframe, investment_amount = 1):
    ...
    port_daily = port_daily.reset_index().drop(['Date'], axis = 1)
